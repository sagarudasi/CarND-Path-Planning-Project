...
// Function to get the input lane and verify whether it is safe to change to that lane
// Return true if its safe otherwise false.
bool is_lane_safe( int clane, auto &sensor_fusion, int prev_size, double car_s ){

	for (int i = 0; i < sensor_fusion.size(); i++)
	{
		//std::cout << "too  close start" << std::endl;
		float d = sensor_fusion[i][6];
		if (d < (2 + 4 * clane + 2) && d > (2 + 4 * clane - 2))
		{
			//std::cout << "too  close start1" << std::endl;
			double vx = sensor_fusion[i][3];
			double vy = sensor_fusion[i][4];
			double check_speed = sqrt(vx * vx + vy * vy);
			double check_car_s = sensor_fusion[i][5];

			//std::cout << "middle lane: "+std::to_string(d) << std::endl;
			check_car_s += ((double)prev_size * .02 * check_speed);

			if ((check_car_s > car_s) && ((check_car_s - car_s) < 25))
			{
				//std::cout << "too  close start2" << std::endl;
				return false;
			}
		}
	}
				...
				if (event == "telemetry")
				{
					...
					if (too_close)
					{
						...
						if( ref_vel < 40 ){
							...
							// If we are in lane 2 then attemp lane 1
							if (lane == 2 || lane == 0)
							{
						...
						//std::cout << "L:" << lane << " lane_change:" << lane_change << " V:" << ref_vel << std::endl;
						//string myString = std::to_string("lane_change:") + std::to_string(",L:") + std::to_string(lane) + std::to_string(",V:") + std::to_string(ref_vel)
					}
					//std::cout << "ref_vel: "+std::to_string(ref_vel)<< std::endl;
					...
					//Define the actual (x,y) points for planner
					...
					// Fill up only remaining points (If pacmac ate 4 points then only generate 4 additional points)
